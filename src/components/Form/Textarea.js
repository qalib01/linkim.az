...
function Textarea({ id, error, label, maxLength, value, ...props }) {
    return (
        <div className="position-relative">
            ...
            { error && <span className="text-danger" style={{ fontSize: '14px' }}> { !maxLength || value.trim().length <= maxLength ? `Zəhmət olmasa, ${label} hissəsini düzgün daxil edin!` : `Maksimum xarakter limiti ${maxLength} olmalıdır!` } </span> }
